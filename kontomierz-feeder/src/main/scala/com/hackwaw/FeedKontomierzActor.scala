  ...
  def addNewBitcoinTransaction(ua: UserAccount) {
    ...
    response.map(ar => {
      ...
      for ( t <-newTransactions) {
        ...
        postNewTransaction(Transaction(BigDecimal(amount) / 1000000, ua.walletId, s"${ar.address} <-> ${t.inputs(0).prev_out.addr}",
          uuid = s"${ar.address}-${t.hash}", isDeposite = isDeposite),
          ua.apiKey)
      }
      // TODO update lastUpdate in all updated transactions
    }
    )
  }
  ...
  def postNewTransaction(tx: Transaction, apiKey: String): Future[CompleteTx] = {
    ...
    kontomierzPipeline(Post(s"https://kontomierz.pl/k4/money_transactions.json?api_key=$apiKey",
      FormData(Seq("money_transaction[currency_amount]"-> tx.amount.toString(),
        ...
        "money_transaction[name]" -> tx.title,
...
case class Transaction(amount: BigDecimal, walletId: Long, title: String, currency: String = "BTC", uuid: String = UUID.randomUUID().toString, isDeposite: Boolean)
